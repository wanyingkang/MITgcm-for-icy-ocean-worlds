...
def tr2d(a):
    ...
    return a.transpose(list(range(a.ndim-2)) + [-1,-2])
...
class Exchange(object):
    ...
    def coords(self, x, y, f, ncs, x0=0, fdest=None, ravel=False):
        '''
        If fdest is given, coordinates are mapped from facet f to fdest
        and new coordinates returned.  x, y, f are not modified.

        If not, coordinates are mapped inside facets.  If ravel is True,
        a raveled index is returned and x, y and f are not modified.
        Otherwise, x, y and f are modified in place.
        '''
        if fdest is not None:
            ...
            for ff in range(self.nf):
                ...
                msk = (f==ff) & (fdest == fn)
                if rot:
                    xo[msk], yo[msk] = ncs + 2*x0 - y[msk], x[msk] - ncs
                else:
                    xo[msk] = x[msk] - ncs
                fo[msk] = fn
                ...
                msk = (f==ff) & (fdest == fn)
                if rot:
                    xo[msk], yo[msk] = y[msk] - ncs, ncs + 2*x0 - x[msk]
                else:
                    yo[msk] -= ncs
                fo[msk] = fn
        ...
        if ravel:
            return np.reshape((y*self.nf + f)*ncs + x, shape)
