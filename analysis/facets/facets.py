...
#                axis = 1
            if axis in [0,-len(facets)]:
                ...
                if axis is not None and axis > 0:
                    axis -= 1
                res = FacetArray( self.f.reduce(f, axis, dtype) for f in facets )
                if axis is None:
                    res = self.f.reduce(res._facets, None, dtype)
                return res
...
def where(cnd, a=None, b=None):
    ...
    for f in range(len(cnd)):
        idxf = np.where(cnd[f])
        idx[0].append(f*np.ones((len(idxf[0]),),np.int64))
        for i in range(1,nd):
...
class Facets(object):
        ...
        if type(indx) == type(()):
            ...
            if np.iterable(indx[0]):
                ...
                for f in range(self.fa.nfacet):
                    ...
                    res[msk] = self.fa._facets[f][b]
                return res
            else:
                facets = self.fa._facets[indx[0]]
                if type(facets) == type(()):
    ...
    def __getslice__(self,i,j):
        return FacetArray(self.fa._facets[i:j])
    ...
    def __repr__(self):
        return 'Facets(' + ',\n\n       '.join( '(' + str(f).replace('\n','\n       ') + ",dtype='" + str(f.dtype) + "')" for f in self.fa._facets ) + ')'

    def __str__(self):
        return '[' + '\n\n '.join( f.__str__().replace('\n','\n ') for f in self.fa._facets ) + ']'
...
class FacetArray(object):
    ...
    def __init__(self, arrs, masks=None, lazy=False):
        if isinstance(arrs, FacetArray):
            arrs = arrs.F
        if masks is not None:
            self._facets = tuple(np.ma.MaskedArray(arr, mask) for arr,mask in zip(arrs,masks))
        elif lazy:
            self._facets = tuple(arrs)
        else:
            self._facets = tuple(np.asanyarray(arr) for arr in arrs)
        if self._facets:
            self.dtype = self._facets[0].dtype
            for f in self._facets[1:]:
    ...
    @classmethod
    def fromfile(cls, fname, dtype, shape=(), dims=None, halo=0, extrau=0,
                 extrav=0, missing=None, map=-1, astype=None):
        if not (halo or extrau or extrav) and dims is None and missing is None:
            return view(np.fromfile(fname, dtype).astype(astype, copy=False), shape, map)
        if astype is None:
            astype = dtype
        obj = cls.zeros(shape, astype, dims, halo, extrau, extrav, missing)
        print(obj.shape)
    ...
    @classmethod
    def frommds(cls, fname, itr=None, dims=None, halo=0, extrau=0,
                extrav=0, missing=None, map=-1, astype=None):
        '''
        Assume single file
        '''
        from MITgcmutils.mds import findmds, readmeta
        if itr is not None:
#            fname, = findmds(fname, -1)
#        else:
            fname, = findmds(fname, itr)
            fname, = fname
        gdims,i0s,ies,timestep,timeinterval,map2gl,meta = readmeta(fname + '.meta')
        sh = meta['nrecords']
        if sh == [1]:
            sh = []
        nd, = meta['nDims']
        ...
        fname = fname + '.data'
        print(fname, dtype, shape)
        return cls.fromfile(fname, dtype, shape, dims, halo, extrau, extrav, missing, map, astype)
    ...
    def copy(self):
        return self.__class__( f.copy() for f in self._facets )

    @property
    def size(self):
        return sum(f.size for f in self._facets)
    ...
    def contiguize(self):
        ...
        for f in range(self.nfacet):
            sz = self._facets[f].size
            data[off:off+sz] = self._facets[f].flat
    ...
    @property
    def data(self):
        return FacetArray(f.data for f in self._facets)

    @data.setter
    def data(self, value):
        if hasattr(value, 'facets'):
            for f in range(self.nfacet):
                self._facets[f].data[...] = value.facets[f]
        else:
    ...
    @property
    def mask(self):
        return FacetArray(f.mask for f in self._facets)

    @mask.setter
    def mask(self, value):
        if hasattr(value, 'facets'):
            for f in range(self.nfacet):
                self._facets[f].mask = value.facets[f]
        else:
            for f in self._facets:
                f.mask = value

    def filled(self, fill_value=None):
        return self.__class__( f.filled(fill_value) for f in self._facets )
    ...
    def astype(self, newtype):
        ...
        output = self.__class__( f.astype(newtype) for f in self._facets )
        return output

    def __getitem__(self, indx):
        if hasattr(indx,'facets'):
            if indx.facets[0].dtype == bool:
                return FacetArray( f[i] for f,i in zip(self._facets,indx.facets) )
            else:
                return NotImplemented
        else:
            return FacetArray( a[indx] for a in self._facets )
    ...
    def __setitem__(self, indx, val):
        ...
        if isinstance(indx[0], FacetArray) and indx[0].dtype == bool:
            ...
            if np.ndim(val) == 0:
                for f in range(self.nfacet):
                    self._facets[f][(findx.facets[f],)+pindx] = val
            elif isinstance(val, FacetArray):
                for f in range(self.nfacet):
                    self._facets[f][(findx.facets[f],)+pindx] = val.facets[f]
            else:
                off = 0
                for f in range(self.nfacet):
                    n = self._facets[f][(findx.facets[f],)+pindx].size
                    self._facets[f][(findx.facets[f],)+pindx] = val[off:off+n]
                    off += n
            return
        ...
        if hasattr(val,'facets'):
            ...
            for f in range(self.nfacet):
                self._facets[f][indx] = val.facets[f]
        else:
            for facet, i in zip(self._facets, indxs):
                facet[i] = val
    ...
    @property
    def shapes(self):
        return [ f.shape for f in self._facets ]
...
#    @property
#    def flat(self):
#        return itools.chain(*(f.flat for f in self._facets))

#    @flat.setter
#    def flat(self, val):
#        val = iter(val)
#        itools.chain(*(f.flat for f in self._facets)) = val
    ...
    def set(self, arr, map=-1, halo=0, extrau=0, extrav=0):
        ...
        for i,s in enumerate(slices):
            ny,nx = self._facets[i].shape[-2:]
            #nyi = ny-2*halo-extrav
            #nxi = nx-2*halo-extrau
            #self[i][..., halo:ny-halo-extrav, halo:nx-halo-extrau].flat = arr[s].reshape(nyi*nxi)
            sh = self._facets[i][..., halo:ny-halo-extrav, halo:nx-halo-extrau].shape
            self._facets[i][..., halo:ny-halo-extrav, halo:nx-halo-extrau] = arr[s].reshape(sh)
    ...
    def __repr__(self):
        #return '<FacetArray(' + ', '.join( str(f.shape) for f in self._facets ) + ') at 0x{0:x}>'.format(id(self))
#        return 'FacetArray(' + ',\n\n       '.join( str(f).replace('\n','\n       ') for f in self._facets ) + ')'
        return 'FacetArray(' + ',\n\n       '.join( '(' + str(f).replace('\n','\n       ') + ",dtype='" + str(f.dtype) + "')" for f in self._facets ) + ')'
    ...
    def std(self, axis=None, dtype=None, out=None, ddof=0):
        ...
        if axis is None:
            STOP
        if out is None:
            return FacetArray( f.std(axis=axis, dtype=dtype) for f in arr.facets )
        else:
            for i,f in enumerate(arr.facets):
                out[i] = f.std(axis=axis, dtype=dtype)
            return out

    def sum(self, axis=None, dtype=None, out=None):
        if axis is None:
            res = np.sum([ np.sum(f, dtype=dtype) for f in self._facets ])
            if out is not None:
                out[:] = res
            return res
        elif axis in [0, -len(self._facets) ]:
            # won't work unless facets all have same shape
            return np.add.reduce(self._facets, dtype=dtype, out=out)
        else:
            if axis > 0:
                axis = axis - 1
            if out is None:
                return FacetArray( f.sum(axis=axis, dtype=dtype) for f in self._facets )
            else:
                for i,f in enumerate(self._facets):
    ...
    def __iadd__(self, other):
        ...
        try:
            ...
            for f,of in zip(self._facets, facets):
                f.__iadd__(of)
        return self
    #
    ...
    def addhalo(self,extra=[]):
        ...
        extra = np.array((self.ndim-len(extra))*[0] + list(extra), int)
        l = []
        for sh, f in zip(self.shapes, self._facets):
            a = np.ndarray.__new__(f.__class__, sh+2*extra, f.dtype)
            l.append(a)
        res = self.__class__(l)
        #res = self.__class__( np.ndarray.__new__(f.__class__, sh+2*extra, f.dtype) for sh,f in zip(self.shapes,self._facets) )

        s = tuple( e and np.s_[e:-e] or np.s_[:] for e in extra )
        for i in range(self.nfacet):
            res.facets[i][s] = self._facets[i]
    ...
    def toglobal(self,out=None,dtype=None,map=-1,halo=0,extrau=0,extrav=0):
        ...
        dimlists = list(zip(*facetshapes))
        gshape,slices = globalmap(facetshapes,map)
        if out is None:
            res = np.zeros(gshape,dtype).view(self._facets[0].__class__)
        else:
            if out.ndim == 1:
...
#                    print flatbounds
        ...
        for s,f in zip(slices,self._facets):
            if map >= 0:
    ...
    def transpose(self, *axes):
        return FacetArray([f.transpose(*axes) for f in self._facets])

    def __array__(self, dtype=None):
        mp = self._map if hasattr(self, '_map') else 1
        return self.toglobal(dtype=dtype, map=mp)
...
def concatenate(l, axis=0):
    n = l[0].nfacet
    return FacetArray([np.concatenate([a.F[f] for a in l], axis) for f in range(n)])
...
def llcdims(n):
    return [n,3*n,n,3*n,n,n,3*n,n,3*n,n]
